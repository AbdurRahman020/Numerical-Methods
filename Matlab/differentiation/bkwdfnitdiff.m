...
Return:
    bkwdifs : two values of derivative, second value is more accurate
...
%}
    ...
    elseif d == 3
        ...
        bd2 = (3*f(x-4*h) - 14*f(x-3*h) + 24*f(x-2*h) - 18*f(x-h) ...
            + 5*f(x)) / (2*h^3);
    elseif d == 4
        bd1 = (f(x-4*h) - 4*f(x-3*h) + 6*f(x-2*h) - 4*f(x-h) + f(x)) / h^4;
        bd2 = (-2*f(x-5*h) + 11*f(x-4*h) - 24*f(x-3*h) + 26*f(x-2*h) ...
            - 14*f(x-h) + 3*f(x)) / h^4;
    else
